...
(function($) {
	...
	var ftGrid = function(container, settings) {
		...
		this.nbChildren = 0;
		this.maxCol = 0;
		this.nbCompactElements = this.nbCompactFailed = this.sameCompactOccurence = 0;
		this.debugData = '<h1>tGrid error</h1>';
		this.boxClasses = '';
		this.lastUpdateFctID = 0;
		this.isInit = true;
		this.verif = false;
		this.spacingType = '%';
		this.errors = {};
	...
	ftGrid.prototype={
		...
		init:function() {
			...
			// - Check if elements must be ignored, set the z-index and define if elements are draggable
		...
		checkClasses:function(boxesClasses) {
			...
			current.$container.children().each(function() {
				...
				if (boxesClasses !== '') {
					...
					var smallBox = $(this).hasClass('tg-small-box'); 					// - too small box bug (below 20px)
					$(this).removeClass().addClass('tg');
					if (smallBox) {
						$(this).addClass('tg-small-box');
					}
			...
			if (typeof current.settings.animation !== typeof undefined && current.settings.animation[0] && current.isInit) {
				...
				switch(animation) {
					...
					case 'Animation-custom' : 
						...
						break;
					default : 
						$element.addClass('tg-na').css({opacity:1});
						break;
				}				
			}
			else
		...
		// - Conztrol that every element are ready for animation
		hasGoodPosition:function(last_count) {
			var current = this,
				count = 0;
			$boxes = current.$container.find('.tg');
			$boxes.each(function() {
				if ($(this).css('top') == '0px' && $(this).css('left') == '0px')
					count++;
			});	
			console.log('goodPosition: ' + count);
			if (count != last_count)	
				current.hasGoodPosition(count);
			return count;
		},
		...
		setContainerHeight:function(speed) {
			...
			var current = this,
				maxHeight; 
			// if (typeof current.settings.auto_fill !== typeof undefined && current.settings.auto_fill[0] && current.nbPerLine > 1)
			// 	maxHeight = current.fillHoles(current.matrix);
			// else {
				var cols = current.getCols();
				maxHeight = cols[cols.length - 1].height;
			// }
		...
		getMesureType:function(element, split) {
		...
		// - Define the width of each element of the grid (in pixel or pourcentage)
		setElementsWidth:function() {
			...
			if (typeof elementsSpacing !== typeof undefined && current.nbPerLine > 1) {
				var widthNoSpacing = 0,
					horizontalSpacing = 0;
				var mesureType = current.getMesureType(elementsSpacing);
				if (mesureType = '%') {
		...
		fillHoles:function() {
			...
			var current = this,
				cols = current.getCols(),
				marginTop = +current.$container.find('.tg').last().margin().top,
				maxHeight = +cols[cols.length - 1].height - marginTop,
				matrix = current.matrix

			current.$container.children('.tg-f').each(function() {
			...
			for (var i = 0; i < cols.length - 1; i++) {
				var top = +cols[i].height,
					col = +cols[i].col,
					leftmesureType = current.getMesureType(current.settings.horizontalSpacing, true),
					left = (+current.leftPercentage + +leftmesureType[0]) * col,
					height = +maxHeight - +top,
					$fill;
				if (typeof current.settings.auto_fill !== typeof undefined && current.settings.auto_fill[1] !== '')
					$fill = current.settings.auto_fill[1].clone();
				else
					$fill = $('<div></div>');
				$fill.addClass('tg-f').css({
					height : +height + 'px',
					width : +current.leftPercentage + current.spacingType,
					opacity : 0,
					position: 'absolute'
				});
				current.$container.append($fill);
				current.eAnimation($fill, i + matrix.length);
				var leftmesureType = current.getMesureType(current.settings.horizontalSpacing, true);
				$fill.css({
					opacity : 1,
					height : +height + 'px',
					width : +current.leftPercentage + '%',
					top : +top + (marginTop/2) + 'px',
					left : left + '%'
				});
			}
			return maxHeight;
	...
	$.fn.tGrid.defaults = {
		...
		boxClasses : [
			'tg-ct-light tg-c-overlayLeft ts-h-scaleIn',		// - classes each size
